...
# @Date:   2014-10-30 00:37:58
# @Last Modified by:   darrenxyli
# @Last Modified time: 2014-10-30 00:43:28
...
class InQueueTask(DictMixin):
    __slots__ = ('taskid', 'priority', 'exptime')
    __getitem__ = lambda *x: getattr(*x)
    __setitem__ = lambda *x: setattr(*x)
    keys = lambda self: self.__slots__

    def __init__(self, taskid, priority=0, exptime=0):
        self.taskid = taskid
        self.priority = priority
        self.exptime = exptime

    def __cmp__(self, other):
        if self.exptime == 0 and other.exptime == 0:
...
class TaskQueue(object):
    ...
    def _check_time_queue(self):
        now = time.time()
        self.mutex.acquire()
        while self.time_queue.qsize() and self.time_queue.top.exptime < now:
    ...
    def _check_processing(self):
        now = time.time()
        self.mutex.acquire()
        while self.processing.qsize() and self.processing.top.exptime < now:
    ...
    def put(self, taskid, priority=0, exptime=0):
        now = time.time()
        self.mutex.acquire()
    ...
    def get(self):
        ...
        try:
            ...
            self.mutex.release()
            return None
        task.exptime = now + self.processing_timeout
